...
public class UpdateUnitServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            WeightUnit u = ud.getUnitById(id);
            session.setAttribute("unit", u);
            request.getRequestDispatcher("update-unit.jsp").forward(request, response);
    ...
    throws ServletException, IOException {
        String id_raw = request.getParameter("idUnit");
        String name = request.getParameter("nameUnit"); 
        int id;
        WeightUnitDAO ud = new WeightUnitDAO();
        try{
            id = Integer.parseInt(id_raw);
            WeightUnit uNew = new WeightUnit(id,name);
            ud.updateCategory(uNew);
